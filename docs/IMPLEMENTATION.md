...
- Shared feed registry supports RSS 2.0, Atom and OPML import, HTML alternate-link discovery, URL aliases, GUID/link/fingerprint fallback, responses bounded to 5 MB, dangerous XML construct rejection and sanitized article HTML including HTTPS images.
...
- Production smoke passed for the SPA and vendored Preact/HTM ESM, anonymous API rejection, Fever `auth: 0`, and the Queue heartbeat. D1 showed the initial heartbeat advance from sequence 0 to 1, then advance itself to sequence 2 five minutes later with no HTTP bootstrap; both runs released their lease.
