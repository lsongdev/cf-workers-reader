...
function Settings({ auth }) {
  ...
  return html`<${Shell} user=${auth.user} csrf=${auth.csrf}><section class="settings"><p class="eyebrow">Reader clients</p><h1>Fever API</h1><p>Connect a Fever-compatible RSS client without sharing your my.lsong.org password.</p>${error&&html`<p class="alert error">${error}</p>`}${credential===undefined?html`<p>Loading…</p>`:credential?html`<div class="credential"><p><strong>Active client access</strong><br/>Username: <code>${credential.username}</code><br/>Created: ${new Date(credential.created_at*1000).toLocaleString()}${credential.last_used_at&&html`<br/>Last used: ${new Date(credential.last_used_at*1000).toLocaleString()}`}</p><button onClick=${create}>Rotate credentials</button> <button onClick=${revoke}>Revoke access</button></div>`:html`<button onClick=${create}>Create client credentials</button>`}${created&&html`<div class="credential-result" role="status"><h2>Save these now</h2><p>The password is shown once. Rotating again invalidates it immediately.</p><label>Server URL<input readonly value=${created.endpoint}/></label><label>Username<input readonly value=${created.username}/></label><label>Password<input readonly value=${created.password}/></label></div>`}<p class="muted">In your client, choose Fever and enter the server URL, username, and password above. JSON API version 3 is supported.</p><a href="/">← Back to Reader</a></section><//>`;
}
function App() {
