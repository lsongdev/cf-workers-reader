...
function Skeleton({ kind, rows = 5 }) {
  const status = html`<span class="sr-only">Loading…</span>`;
  if (kind === 'article') return html`<article class="article skeleton-article-page" role="status">${status}<header><span class="skeleton-shape skeleton-kicker"></span><span class="skeleton-shape skeleton-title"></span><span class="skeleton-shape skeleton-title short"></span><span class="skeleton-shape skeleton-meta"></span><div class="skeleton-actions">${[0, 1, 2].map(index => html`<span class="skeleton-shape" key=${index}></span>`)}</div></header><div class="skeleton-copy">${[92, 100, 87, 96, 68, 94, 82].map((width, index) => html`<span class="skeleton-shape" style=${`width:${width}%`} key=${index}></span>`)}</div></article>`;
  if (kind === 'credential') return html`<div class="credential skeleton-credential" role="status">${status}<span class="skeleton-shape skeleton-line medium"></span><span class="skeleton-shape skeleton-line"></span><span class="skeleton-shape skeleton-line short"></span><span class="skeleton-shape skeleton-button"></span></div>`;
  const article = kind === 'articles', directory = kind === 'directory';
  return html`<div class=${directory ? 'directory-grid skeleton-list' : article ? 'article-list skeleton-list' : 'feed-list skeleton-list'} role="status">${status}${Array.from({ length: rows }, (_, index) => html`<div class=${directory ? 'directory-card skeleton-directory-row' : article ? 'article-row skeleton-article-row' : 'feed-row skeleton-feed-row'} key=${index}>${!article && html`<span class="skeleton-shape skeleton-avatar"></span>`}<span class="skeleton-text"><span class="skeleton-shape skeleton-line medium"></span><span class="skeleton-shape skeleton-line"></span></span><span class="skeleton-shape skeleton-tail"></span></div>`)}</div>`;
}

function BootSkeleton() {
  return html`<div class="app-shell skeleton-boot" role="status"><span class="sr-only">Opening Reader…</span><header class="navbar"><span class="navbar-brand"><img src="/icon-192.png" width="24" height="24" alt=""/>Reader</span><span class="skeleton-shape skeleton-account"></span></header><main><section class="view"><span class="skeleton-shape skeleton-kicker"></span><span class="skeleton-shape skeleton-title short"></span><span class="skeleton-shape skeleton-description"></span><${Skeleton} kind="feeds" rows=${4}/></section></main></div>`;
}
...
function FeedsView({ auth }) {
  ...
  return html`<${Shell} user=${auth.user} csrf=${auth.csrf}><section class="view feeds-view">
    ...
    ${feeds === undefined ? html`<${Skeleton} kind="feeds" rows=${5}/>` : !feeds.length ? html`<div class="empty-state"><span>◎</span><h2>Your library is empty</h2><p>Add an RSS feed or a website with a feed to begin.</p></div>` : groups.map(group => {
...
function ArticlesView({ auth }) {
  ...
  return html`<${Shell} user=${auth.user} csrf=${auth.csrf}><section class="view articles-view">
    ...
    ${items === undefined ? html`<${Skeleton} kind="articles" rows=${6}/>` : html`<section class="article-list" aria-label="Articles">${!items.length ? html`<div class="empty-state"><span>✓</span><h2>${filter === 'unread' ? 'You’re all caught up' : 'No articles here'}</h2><p>Choose another collection or return to your subscriptions.</p></div>` : items.map(item => html`<${Link} class=${`article-row ${item.read ? 'is-read' : ''}`} href=${`/article/${item.id}?return=${encodeURIComponent(returnTo)}`} key=${item.id}><span class="article-row-main"><small>${item.feed_title}${item.author ? ` · ${item.author}` : ''}</small><strong>${Boolean(item.starred) && html`<span class="saved-star">★</span>`}${item.title}</strong></span><time>${shortDate(item.published_at)}</time><span class="row-arrow">→</span><//>`)}</section>`}
...
function App() {
  ...
  if (auth === undefined) return html`<${BootSkeleton}/>`;
