...
function ArticlesView({ auth }) {
  ...
  return html`<${Shell} user=${auth.user} csrf=${auth.csrf}><section class="view articles-view">
    ...
    <section class="article-list" aria-label="Articles">${items === undefined ? html`<div class="loading-list">Loading articles…</div>` : !items.length ? html`<div class="empty-state"><span>✓</span><h2>${filter === 'unread' ? 'You’re all caught up' : 'No articles here'}</h2><p>Choose another collection or return to your subscriptions.</p></div>` : items.map(item => html`<${Link} class=${`article-row ${item.read ? 'is-read' : ''}`} href=${`/article/${item.id}?return=${encodeURIComponent(returnTo)}`} key=${item.id}><span class="article-row-main"><small>${item.feed_title}${item.author ? ` · ${item.author}` : ''}</small><strong>${item.starred && html`<span class="saved-star">★</span>`}${item.title}</strong></span><time>${shortDate(item.published_at)}</time><span class="row-arrow">→</span><//>`)}</section>
